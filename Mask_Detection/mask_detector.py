...
def maskdetection(path):
    ...
    if not check:
      ...
      destpathalgo= os.path.join(destination,foldername)
      check_algopath = os.path.isdir(destpathalgo)
      if not check_algopath:
        makedirs(destpathalgo)
      else:
        os.rename(destpathalgo, destpathalgo+ " "+ timestamp)
        os.makedirs(destpathalgo)    
    else:
        destpathalgo= os.path.join(destination,foldername)
        check_algopath = os.path.isdir(destpathalgo)
        if not check_algopath:
           makedirs(destpathalgo)
            ...
            os.rename(destpathalgo, destpathalgo+ " "+ timestamp)
            os.makedirs(destpathalgo)
    filepath=os.path.join(destpathalgo,"log.txt")
    video_capture=cv2.VideoCapture(path)
    i = 1
    width= int(video_capture.get(cv2.CAP_PROP_FRAME_WIDTH))
    height= int(video_capture.get(cv2.CAP_PROP_FRAME_HEIGHT))
    videopath = os.path.join(destpathalgo,foldername)
            ...
            for (x,y,w,h) in faces:
                ...
                if type(face) is np.ndarray:
                    ...
                    if(pred[0][0]>=1):
                        ...
                        sourceFile = open(filepath, 'a')
                        print("mask  {}".format(seconds),file = sourceFile)
                        ...
                        seconds = time_milli / 1000
                        sourceFile = open(filepath, 'a')
                        print("no mask  {}".format(seconds),file = sourceFile)
